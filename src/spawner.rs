...
use crate::{
    config::{DEFAULT_REGION_HEIGHT, DEFAULT_REGION_WIDTH, FluidSettings},
    particle::{Particle, Position, RegionBoundingBox, Velocity},
};
...
impl Plugin for Spawner {
    fn build(&self, app: &mut App) {
        app.insert_resource(SpawnerSettings::default())
            .add_systems(Update, draw_spawn_gizmos)
...
fn spawn_particles(
    ...
    for (_, region) in settings.spawn_regions.iter().enumerate() {
        ...
        for point in points {
            ...
            let position = (point + jitter).extend(0.0) / fluid_settings.pixels_per_meter;
